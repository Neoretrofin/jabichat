...
- **Opus SDP munging для hi-fi звука.** `enhanceOpusSdp()` патчит `a=fmtp:N` строку Opus в SDP перед `setLocalDescription`: `stereo=1; sprop-stereo=1; maxaveragebitrate=128000; usedtx=0; cbr=1; maxplaybackrate=48000; useinbandfec=1`. Применяется и к offer'у, и к answer'у в `useCallController` — иначе hi-fi работает только в одну сторону. Без этого по дефолту WebRTC шлёт Opus в voice-mode (моно ~32 kbps, DTX режет тишину между нотами) — музыка/фильм через screen-share звучат «как телефон». Параметры дескриптивны: каждая сторона декларирует «как меня кодировать» в своей половине SDP, поэтому обе стороны должны мунжить свою SDP. Идемпотентно (мерж с существующими fmtp-параметрами). Не трогать функцию `enhanceOpusSdp` лёгкой рукой — это тонкая string-патч-логика, тестировать на десктоп↔мобильном звонке после изменений.
